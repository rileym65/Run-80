...
int evaluate(struct Z80CPU *cpu, char* expr) {
  int  i;
  word value;
  word nstack[256];
  byte ostack[256];
  ...
  while (*expr != 0 && op != OP_END) {
    ...
    else if (strncasecmp(expr, "bc", 2) == 0) {value = cpu->BC.BC; expr += 2; }
    else if (strncasecmp(expr, "de", 2) == 0) {value = cpu->DE.DE; expr += 2; }
    else if (strncasecmp(expr, "hl", 2) == 0) {value = cpu->HL.HL; expr += 2; }
    else if (strncasecmp(expr, "sp", 2) == 0) {value = cpu->SP; expr += 2; }
    else if (strncasecmp(expr, "ix", 2) == 0) {value = cpu->IX; expr += 2; }
    else if (strncasecmp(expr, "iy", 2) == 0) {value = cpu->IY; expr += 2; }
    else if (*expr == 'a' || *expr == 'A') { value = cpu->AF.AFu.A; expr++; }
    else if (*expr == 'f' || *expr == 'F') { value = cpu->AF.AFu.F; expr++; }
    else if (*expr == 'b' || *expr == 'B') { value = cpu->BC.BCu.B; expr++; }
    else if (*expr == 'c' || *expr == 'C') { value = cpu->BC.BCu.C; expr++; }
    else if (*expr == 'h' || *expr == 'H') { value = cpu->HL.HLu.H; expr++; }
    else if (*expr == 'l' || *expr == 'L') { value = cpu->HL.HLu.L; expr++; }
    else {
